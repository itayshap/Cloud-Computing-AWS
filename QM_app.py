...
def load_balancing():
    while True:
        middle_work = int(len(queue) / 2)
        if int(time.time() - middle_work['time']) > AUTO_SCALE_TIME:
                spawn_worker()

app = Flask(__name__)
queue = [{'work_id' : 1, 'time': time.time(), 'work': "hello world", 'iterations' : 200}]
completed_work = []
...
def enqueue():
    ...
    new_work = {'work_id' :work_counter, 'time': time.time(), 'work': work}
...
def pullCompleted():
    ...
    return json.dumps(queue[0:top], indent=2)
